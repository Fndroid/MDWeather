package com.fndroid.byweather.utils;
...
public class Util {
	...
	public static byte[] bmpToByteArray2(final Bitmap bmp, final boolean needRecycle) {
		...
		if (i < 100) {
			...
			bmp.compress(CompressFormat.JPEG, i, output);// 这里压缩options%，把压缩后的数据存放到baos中
		}
		if (needRecycle) {
			bmp.recycle();
		}
	...
	public static Bitmap extractThumbNail(final String path, final int height, final int width,
		...
		try {
			...
			options.inSampleSize = (int) (crop ? (beY > beX ? beX : beY) : (beY < beX ? beX :
					beY));
